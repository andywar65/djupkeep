window.addEventListener("map:init", function (event) {
    ...
    if (unit !== "" ) {
        ...
        for (let i = 1; i < (image_size[0]-origin[0])/length; i++) {
            L.polyline([[0, i*length], [xtrim, i*length]], {weight: 1}).bindPopup("X="+i.toString()).addTo(map);
        }
        for (let i = 1; i < (image_size[1]-origin[1])/length; i++) {
            L.polyline([[i*length, 0], [i*length, ytrim]], {weight: 1}).bindPopup("Y="+i.toString()).addTo(map);
        }
    }
    // Add axis
    if (origin !== "" ) {
